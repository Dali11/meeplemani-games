...
export default async function InboxPage({
    ...
    <div className="wrap py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <form method="get" action="/admin" role="search" className="flex gap-2">
          ...
          <input
            ...
            className="w-64 rounded-full border border-line-strong bg-white/5 px-4 py-2 text-[15px] text-cream placeholder:text-dim focus:border-lamp focus:outline-none"
          />
          <button
            type="submit"
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                className={`inline-flex min-h-10 items-center gap-2 rounded-full border px-4 text-[15px] transition-colors ${
                  active
          ...
          {[
            ...
            <Link
              ...
              className={`rounded-full px-3 py-1.5 transition-colors ${
                kind === k.value ? "bg-white/10 text-cream" : "text-muted hover:text-cream"
              }`}
            >
              {k.label}
            </Link>
                        ...
                        {STATUSES.map((s) => (
                          <form key={s} action={updateInquiryStatus}>
                            ...
                            <button
                              type="submit"
                              disabled={current === s}
                              className={`min-h-9 rounded-full border px-3.5 text-[14px] transition-colors ${
                                current === s
                                  ? "border-cream bg-cream font-medium text-[#1b1630]"
                                  : "border-line-strong text-muted hover:text-cream"
                              }`}
